...
def draw_dirs2(filelist, column_name):
    ...
    #print(sum(sum(sum(res_dir_x_list_HL))))
    #print(sum(sum(sum(res_dir_x_list_LH))))
...
"""
            ...
            if (d==0 or d==8):
                res_dir_x += dir * np.cos(0)
                res_dir_y += dir * np.sin(0)
            elif (d == 2 or d == 10):
                res_dir_x += dir * np.cos(np.pi / 2)
                res_dir_y += -dir * np.sin(np.pi / 2)
            elif(d == 4 or d == 12):
                res_dir_x += -dir * np.cos(0)
                res_dir_y += dir * np.sin(0)
            elif (d==6 or d==14):
                ...
                res_dir_x += dir * np.cos(np.pi / 4)
                res_dir_y += -dir * np.sin(np.pi / 4)
            elif (d == 3 or d == 11):
                res_dir_x += -dir * np.cos(np.pi / 4)
                res_dir_y += -dir * np.sin(np.pi / 4)
            elif (d == 5 or d == 13):
                ...
                res_dir_x += dir * np.cos(np.pi / 8)
                res_dir_y += -dir * np.sin(np.pi / 8)
            elif (d == 17):
                res_dir_x += dir * np.cos(3 * np.pi / 8)
                res_dir_y += -dir * np.sin(3 * np.pi / 8)
            elif (d == 19):
                res_dir_x += -dir * np.cos(np.pi / 8)
                res_dir_y += -dir * np.sin(np.pi / 8)
            elif (d == 18):
...
def createWeightedGraph(contourdf, file_list, column_name):
    ...
    #print(data['res_x_HL'].sum())
    #print(merged_df['res_x_HL'].sum())
    #print(data['res_x_LH'].sum())
    #print(merged_df['res_x_LH'].sum())
    ...
    weighted_df['res_dir_x_1_HL'] = weighted_df.groupby(['level', 'path'])['res_dir_x_1_HL'].transform('sum') / (weighted_df[
        'aggregated_weight']+.00000001)
    weighted_df['res_dir_y_1_HL'] = weighted_df.groupby(['level', 'path'])['res_dir_y_1_HL'].transform('sum') / (weighted_df[
        'aggregated_weight']+.00000001)
    weighted_df['res_dir_x_1_LH'] = weighted_df.groupby(['level', 'path'])['res_dir_x_1_LH'].transform('sum') / (weighted_df[
        'aggregated_weight']+.00000001)
    weighted_df['res_dir_y_1_LH'] = weighted_df.groupby(['level', 'path'])['res_dir_y_1_LH'].transform('sum') / (weighted_df[
        'aggregated_weight']+.00000001)
    ...
    #print(weighted_df['mag_HL'].max())
    #print(weighted_df['mag_LH'].max())
