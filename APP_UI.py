...
### ISOLINES#############
    ...
    def setHorizontalMagnitude(self):
        ...
        self.horizontalSlider_magnitude.setMinimum(max(0,min(int(magnitude_HL[np.isfinite(magnitude_HL)].min()),int(magnitude_LH[np.isfinite(magnitude_LH)].min()))))
        self.horizontalSlider_magnitude.setMaximum(min(1000,max(int(magnitude_HL[np.isfinite(magnitude_HL)].max()),int(magnitude_LH[np.isfinite(magnitude_LH)].max()))))
...
## EDIT ##############
    ...
    def draw_diagram(self):
        ...
        self.canvas.generate_images(self.filtered_graph,list(self.filenameToPathDict.values())[-1],self.data,self.getcutOffValues(),column,alpha,self.directionCheckBoxHandler(),
